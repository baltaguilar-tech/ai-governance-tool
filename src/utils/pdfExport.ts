...
export async function generateProPDF(
  ...
  for (const dimension of DIMENSIONS) {
    ...
    // If this dimension was not assessed, show a summary placeholder instead of Q&A
    if (!dimScore?.answered) {
      doc.setFillColor(240, 244, 248);
      doc.roundedRect(margin, yPos, pageWidth - margin * 2, 12 + dimQuestions.length * 6 + 8, 3, 3, 'F');
      doc.setFontSize(9);
      doc.setTextColor(98, 125, 152);
      doc.text('This dimension was not assessed. The following questions were not answered:', margin + 4, yPos + 8);
      dimQuestions.forEach((q, i) => {
        doc.setFontSize(8);
        doc.setTextColor(120, 140, 160);
        const qLines = doc.splitTextToSize(`${i + 1}. ${q.text}`, pageWidth - margin * 2 - 10);
        doc.text(qLines, margin + 6, yPos + 16 + i * 6);
      });
